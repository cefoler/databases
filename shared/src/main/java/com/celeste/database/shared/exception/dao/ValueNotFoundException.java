...
/**
 * The class {@code ValueNotFoundException} is a checked {@code Exception} class that inherits the
 * {@link DAOException} class that is, it is an exception that needs to be catch.
 *
...
public class ValueNotFoundException extends DAOException {
  ...
  /**
   * Constructs a new {@code ValueNotFoundException} with the specified detail message. The cause is
   * not initialized, and may subsequently be initialized by a call to {@link #initCause}.
   *
   * @param error the detail message. The detail message is saved for later retrieval by the
